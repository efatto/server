        ...
        **Note**: The type of field values to pass in ``vals`` for relationship fields is specific.
        ...
        if self._parent_store and not context.get('defer_parent_store_computation'):
            if self.pool._init:
                ...
                # 20141127 - FIX Broken Parent Store with an explicit
                # self._parent_store_compute(cr) call when pleft is None
                while True:
                    ...
                    # 20141127 - FIX Broken Parent Store
                    if pleft is not None:
                        ...
                        _logger.warning("Found a broken parent store in table '%s': recomputing values..." % self._table)
                        self._parent_store_compute(cr)
